...
if(!empty($jwt)){
    try{
        $decoded = JWT::decode($jwt, $key, array('HS256'));   // ถอดรหัส jwt        
            ...
            $sql = "SELECT * FROM shift WHERE ( shift_name LIKE '%".$search."%') ORDER BY shift_name ASC LIMIT $rowStart , $perpage";
                $stmt = $db->prepare( $sql );  
                $stmt->execute();
                $num = $stmt->rowCount();   
                $resultArray = array();
                while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $sql2 = "SELECT shift_id, shift_name FROM shift WHERE shift_name LIKE '%".$search."%'";
